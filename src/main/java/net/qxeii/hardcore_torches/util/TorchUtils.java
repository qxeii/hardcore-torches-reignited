		...
		if (particle != null) {
			if (state.contains(Properties.HORIZONTAL_FACING)) {
				...
				if (world instanceof ServerWorld) {
					((ServerWorld) world).spawnParticles(particle, d + 0.27 * (double) dir2.getOffsetX(), e + 0.22,
							f + 0.27 * (double) dir2.getOffsetZ(), 1, 0, 0, 0, 0);
				} else if (world.isClient) {
					world.addParticle(particle, d + 0.27 * (double) dir2.getOffsetX(), e + 0.22,
							f + 0.27 * (double) dir2.getOffsetZ(), 0.0, 0.0, 0.0);
				}
			} else {
				if (world instanceof ServerWorld) {
