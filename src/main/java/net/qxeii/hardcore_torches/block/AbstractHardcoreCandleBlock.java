...
public abstract class AbstractHardcoreCandleBlock extends BlockWithEntity implements BlockEntityProvider, IFuelBlock {
    ...
    public static boolean isLitCandle(BlockState state) {
        return (Boolean)state.get(LIT) && (state.isIn(BlockTags.CANDLES) || state.isIn(BlockTags.CANDLE_CAKES)) && (Boolean)state.get(LIT);
    }
    ...
    public static void extinguish(@Nullable PlayerEntity player, BlockState state, WorldAccess world, BlockPos pos) {
        ...
        public static void melt(BlockState state, World world, BlockPos pos) {
            ...
            changeCandles(world, state, pos, -1);
                ((AbstractHardcoreCandleBlock)state.getBlock()).getParticleOffsets(state).forEach((offset) -> {
                    world.addParticle(ParticleTypes.SMOKE, (double) pos.getX() + offset.getX(), (double) pos.getY() + offset.getY(), (double) pos.getZ() + offset.getZ(), 0.0D, 0.001D, 0.0D);
                });
    ...
    public static void ignite(@Nullable PlayerEntity player, BlockState state, World world, BlockPos pos) {
        ...
        if (state.getBlock() instanceof CandleBlock) {
            ((AbstractHardcoreCandleBlock)state.getBlock()).getParticleOffsets(state).forEach((offset) -> {
                world.addParticle(ParticleTypes.SMALL_FLAME, (double)pos.getX() + offset.getX(), (double)pos.getY() + offset.getY(), (double)pos.getZ() + offset.getZ(), 0.0D, 0.0D, 0.0D);
            });
        }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, Mod.CANDLE_BLOCK_ENTITY, (world1, pos, state1, be) -> CandleBlockEntity.tick(world1, pos, state1, be));
    }
    //endregion
    @Override
