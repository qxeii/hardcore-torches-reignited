...
public class CandleRecipe extends ShapedRecipe {
    ...
    public CandleRecipe(Identifier id, String group, int width, int height, DefaultedList<Ingredient> input, ItemStack output) {
    ...
    public static class Serializer implements RecipeSerializer<CandleRecipe> {
        ...
        public CandleRecipe read(Identifier resourceLocation, JsonObject json) {
            ...
            return new CandleRecipe(recipe.getId(), recipe.getGroup(), recipe.getWidth(), recipe.getHeight(), recipe.getIngredients(), recipe.getOutput(DynamicRegistryManager.EMPTY));
        }
